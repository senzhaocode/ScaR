...
use Check;
	...
	if (! defined($splice_tag) ) { # judge the setting of $splice_tag for genome level alignment
		...
		if ( $splice_tag eq 'hisat2' ) {
			...
			if (! -e "/data/STAR_index/chrStart.txt" ) { print "Star in file $input/chrStart.txt does not exist\n\n"; exit; }
			if (! -e "/data/STAR_index/chrName.txt" ) { print "Star index file $input/chrName.txt does not exist\n\n"; exit; }
			if (! -e "/data/STAR_index/chrNameLength.txt" ) { print "Star index file $input/chrNameLength.txt does not exist\n\n"; exit; }
			if (! -e "/data/STAR_index/chrLength.txt" ) { print "Star index file $input/chrLength.txt does not exist\n\n"; exit; }
			if (! -e "/data/STAR_index/sjdbInfo.txt" ) { print "Star index file $input/sjdbInfo.txt does not exist\n\n"; exit; }
			if (! -e "/data/STAR_index/genomeParameters.txt" ) { print "Star index file $input/genomeParameters.txt does not exist\n\n"; exit; }
			if (! -e "/data/STAR_index/sjdbList.out.tab" ) { print "Star index file $input/sjdbList.out.tab does not exist\n\n"; exit; }
			if (! -e "/data/STAR_index/Genome" ) { print "Star index file $input/Genome does not exist\n\n"; exit; }
			if (! -e "/data/STAR_index/SA" ) { print "Star index file $input/SA does not exist\n\n"; exit; }
			if (! -e "/data/STAR_index/SAindex" ) { print "Star index file $input/SAindex does not exist\n\n"; exit; }
		} else {
	...
	my $genome_index_star = "/data/STAR_index/"; # set Star index
