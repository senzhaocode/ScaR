...
use Check;
	...
	push @usage, "	--scaffold	A list of fusion scaffold sequences in fasta format, e.g\n", 
		     ...
		     "			>scaffold2\n",
		     "			GCTCTATGAAATTGCA*AACAAAGAGAGGGTCA\n";
	push @usage, "	--input		Set input annotation files for running script: e.g. /script_path/data/\n";
	...
	defined $input or die @usage; if (! -e "$input/transcript_cdna.fa" ) { print "cDNA sequence file $input/transcript_cdna.fa does not exist\n\n"; exit; } 
	...
	if (! defined($trimm) ) { 
		...
		if ( $trimm == 0 ) {
			...
			if ( $read_length == 0 ) {
				print "the input read length is missing or invalid, pleaes input correct number\n"; exit;
			}
		}
	}
	...
	my $tran_seq = "$input/transcript_cdna.fa"; # cdna mRNA sequence files
...
print "############################################################################################\n\n";
	...
	if ( %scaff_seq ) {
		foreach my $name ( keys %scaff_seq ) { # $name => name of breakpoint sequence
			...
			print "############################################################################################\n";
			my ($tag_scaff_geneA, $tag_scaff_geneB, $ref_sequence) = Check::judge_scaffold($scaff_seq{$name}, $tran_seq, $gene, $geneA, $geneB);
			...
			if ( $tag_scaff_geneA == 1 ) {
				if ( $tag_scaff_geneB == 1 ) {
					...
					open (OUT, ">$output/$name/scaffold_${transA}_${breakpoint_scaffold}_${transB}_${read_length}_seq.fa") || die "cannot open this path for scaffold sequence:$!\n";
					print OUT ">$geneA\n";
			...
			$hisat_flag == 0 or die "2-1 Hisat2 mapping to GeneA-GeneB-scaffold fails\n";
			...
			Extract::Discordant(\%discordant, $anchor, $breakpoint_scaffold, $read_length, "$output/$name/");
			...
			if (! -z "$output/$name/read_mapped_info" ) { #whether read_mapped_info is zero
				...
				if (! -z "$output/$name/final_read_mapped_info" ) { # final_read_mapped_info is not zero => put spanning and split reads together for running alignment
					`cat $output/$name/final_spanning_1.txt > $output/$name/tmp/final_read_1.txt`; `cat $output/$name/final_split_1.txt >> $output/$name/tmp/final_read_1.txt`;
					`cat $output/$name/final_spanning_2.txt > $output/$name/tmp/final_read_2.txt`; `cat $output/$name/final_split_2.txt >> $output/$name/tmp/final_read_2.txt`;
					my $final_flag = system("hisat2 -p $cpus --no-unal --no-spliced-alignment --no-softclip -x $output/$name/tmp/index -q -1 $output/$name/tmp/final_read_1.txt -2 $output/$name/tmp/final_read_2.txt -S $output/$name/tmp/final_noclip.sam"); # name of sam file
        				$final_flag == 0 or die "Step 3-4 Hisat2 map to GeneA-GeneB-scaffold fails\n";
					# 4.4 transform sam format to sam format using samtools
					if ( -e "$output/$name/tmp/final_noclip.sam" ) {
						`samtools view $output/$name/tmp/final_noclip.sam -bS > $output/$name/tmp/final_noclip.bam`;
						`samtools sort $output/$name/tmp/final_noclip.bam -o $output/$name/final_noclip.sorted.bam`;
						`samtools index $output/$name/final_noclip.sorted.bam`;
						`rm $output/$name/tmp/final_noclip.bam`;
						`rm $output/$name/tmp/final_read_*.txt`;
					}
				}
			} else {
