	...
	if ( %scaff_seq ) {
		foreach my $name ( keys %scaff_seq ) { # $name => name of breakpoint sequence
			...
			if (! -z "$output/$name/read_mapped_info" ) { #whether read_mapped_info is zero
				...
				if ( $Per_Dis_A == 0 ) { $Per_Dis_A = 0; } else { $Per_Dis_A = $Dis_A_F/$Dis_A; }
				if ( $Per_Dis_B == 0 ) { $Per_Dis_B = 0; } else { $Per_Dis_B = $Dis_B_F/$Dis_B; }
				if ( $Per_Spa_AB == 0 ) { $Per_Spa_AB = 0; } else { $Per_Spa_AB = $Spa_AB_F/$Spa_AB; }
